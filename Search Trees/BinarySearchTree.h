...
public: 						    // public types
    typedef typename E::Key K;				// a key    E represents Entry class
    typedef typename E::Value V;			// a value  E represents Entry class
...
// begin() returns the first node(the leftmost internal node) 
...
// On success, return found position v 
// Otherwise, return found external position
template <typename E>					// find utility
typename SearchTree<E>::TPos SearchTree<E>::finder(const K& k, const TPos& v) {
    if (v.isExternal()) return v;			// key not found because we only store entries in internal nodes
    if (k < v->key()) return finder(k, v.left());	// search left subtree
    else if (v->key() < k) return finder(k, v.right());	// search right subtree
    else return v;					// else v->key() == k, that means we find the key here
}
